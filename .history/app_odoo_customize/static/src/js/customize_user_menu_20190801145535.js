odoo.define('app_odoo_customize.customize_user_menu', function (require) {
    ...
    $(document).ready(function () {
        ...
        setTimeout(function () {
            new rpc('ir.config_parameter').call('search_read', [[['key', '=', 'app_show_debug']], ['value']]).then(function (show) {
            ...
            new rpc('ir.config_parameter').call('search_read', [[['key', '=', 'app_show_documentation']], ['value']]).then(function (show) {
                if (show.length >= 1 && (show[0]['value'] == "False"))
                    $('[data-menu="documentation"]').parent().hide();
                else {
                    new rpc('ir.config_parameter').call('search_read', [[['key', '=', 'app_documentation_url']], ['value']]).then(function (res) {
                        if (res.length >= 1) {
            ...
            new rpc('ir.config_parameter').call('search_read', [[['key', '=', 'app_show_documentation_dev']], ['value']]).then(function (show) {
                if (show.length >= 1 && (show[0]['value'] == "False"))
                    $('[data-menu="documentation_dev"]').parent().hide();
                else {
                    new rpc('ir.config_parameter').call('search_read', [[['key', '=', 'app_documentation_dev_url']], ['value']]).then(function (res) {
                        if (res.length >= 1) {
                            _.each(res, function (item) {
            ...
            new rpc('ir.config_parameter').call('search_read', [[['key', '=', 'app_show_support']], ['value']]).then(function (show) {
                if (show.length >= 1 && (show[0]['value'] == "False"))
                    $('[data-menu="support"]').parent().hide();
                else {
                    new rpc('ir.config_parameter').call('search_read', [[['key', '=', 'app_support_url']], ['value']]).then(function (res) {
                        if (res.length >= 1) {
                            _.each(res, function (item) {
            ...
            new rpc('ir.config_parameter').call('search_read', [[['key', '=', 'app_show_account']], ['value']]).then(function (show) {
                if (show.length >= 1 && (show[0]['value'] == "False"))
                    $('[data-menu="account"]').parent().hide();
                else {
                    new rpc('ir.config_parameter').call('search_read', [[['key', '=', 'app_account_title']], ['value']]).then(function (res) {
                        if (res.length >= 1) {
                            _.each(res, function (item) {
            ...
            new rpc('ir.config_parameter').call('search_read', [[['key', '=', 'app_account_url']], ['value']]).then(function (res) {
                if (res.length >= 1) {
                    _.each(res, function (item) {
            ...
            new rpc('ir.config_parameter').call('search_read', [[['key', '=', 'app_show_poweredby']], ['value']]).then(function (show) {
                if (show.length >= 1 && (show[0]['value'] == "False"))
