...
class warehouse(models.Model):
    ...
    def get_warehouse_by_type(self, _type):
        ...
        if not _type or _type not in map(lambda _type: _type[0], self.WAREHOUSE_TYPE):
            raise UserError(u'错误', u'仓库类型" % s"不在预先定义的type之中，请联系管理员' % _type)

        warehouses = self.search([('type', '=', _type)], limit=1, order='id asc')
        if not warehouses:
            raise UserError(u'错误', u'不存在该类型" % s"的仓库，请检查基础数据是否全部导入')
...
class pricing(models.Model):
    ...
    def get_pricing_id(self, partner, warehouse, goods, date):
        ...
        '''
        if not partner:
            raise UserError(u'错误',u'请先输入客户')
        if not warehouse:
            raise UserError(u'错误',u'请先输入仓库')
        if not goods:
            raise UserError(u'错误',u'请先输入商品')
        # 客户类别、仓库、产品满足条件
...
class res_users(models.Model):
    ...
    def write(self, vals):
        ...
        if self.env.user.id != 1:
            for record in self:
                if record.id == 1:
                    raise UserError(u'错误', u'系统用户不可修改')
        # 如果管理员将自己的系统管理权限去掉，则报错
        else:
            if not self.env.user.has_group('base.group_erp_manager'):
                raise UserError(u'错误', u'不能删除管理员的管理权限')
        return res
