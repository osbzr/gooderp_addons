odoo.define('gooderp_pos.gui', function (require) {
"use strict";
...
var Gui = core.Class.extend({
    ...
    _close: function() {
        ...
        this.pos.push_order().then(function(){
            var url = "/web#action=gooderp_pos.action_client_pos_menu";
    ...
    play_sound: function(sound) {
        var src = '';
        if (sound === 'error') {
            src = "/gooderp_pos/static/src/sounds/error.wav";
        } else if (sound === 'bell') {
            src = "/gooderp_pos/static/src/sounds/bell.wav";
        } else {
    ...
    download_file: function(contents, name) {
        var href_params = this.prepare_file_blob(contents,name);
