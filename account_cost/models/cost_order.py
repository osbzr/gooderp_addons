...
class CostOrder(models.Model):
    ...
    partner_id = fields.Many2one('partner', '供应商', states=READONLY_STATES,
                                 ondelete='restrict',
                                 help='供应商')
    ...
    name = fields.Char('单据编号', index=True, copy=False,
                       help="服务订单的唯一编号，当创建时它会自动生成下一个编号。")
    line_ids = fields.One2many('cost.order.line', 'order_id', '服务明细行',
                               states=READONLY_STATES, copy=True,
                               help='采购服务的明细行，不能为空')
    note = fields.Text('备注', help='单据备注')
    prepayment = fields.Float('预付款', states=READONLY_STATES,
                              digits=dp.get_precision('Amount'),
                              help='输入预付款确认购货订单，会产生一张付款单')
    ...
    wh_move_ids = fields.Many2many('wh.move',
                                   ...
                                   'cost_ids',
                                   'move_ids',
                                   '费用分摊出入库明细', copy=False)
    approve_uid = fields.Many2one('res.users', '确认人',
                                  copy=False, ondelete='restrict',
                                  help='确认单据的人')
    state = fields.Selection(COST_ORDER_STATES, '确认状态', readonly=True,
                             help="单据的确认状态", index=True, copy=False,
                             default='draft')
    amount = fields.Float('合计金额', store=True, readonly=True,
                          compute='_compute_amount', track_visibility='always',
                          digits=dp.get_precision('Amount'),
                          help='总金额减去优惠金额')
    tax_amount = fields.Float('合计税额', store=True, readonly=True,
                              compute='_compute_amount', track_visibility='always',
                              digits=dp.get_precision('Amount'))
    ...
    def cost_order_confim(self):
        ...
        if self.state == 'cancel':
            raise UserError('请不要确认已中止的订单！')
        if not self.line_ids:
            raise UserError('请输入服务明细行！')
...
class CostOrderLine(models.Model):
    ...
    order_id = fields.Many2one('cost.order', '订单编号', index=True,
