...
class SellOrder(models.Model):
    ...
    delivery_date = fields.Date(
        ...
        copy=False,
        help="订单的要求交货日期")
    type = fields.Selection([('sell', '销货'), ('return', '退货')], '类型',
                            default='sell', states=READONLY_STATES,
                            help='销货订单的类型，分为销货或退货')
    ...
    line_ids = fields.One2many('sell.order.line', 'order_id', '销货订单行',
    ...
    def sell_order_draft(self):
        ...
        if self.goods_state != '未出库':
            raise UserError('该销货订单已经发货，不能撤销确认！')
        # 查找产生的发货单并删除
    ...
    def sell_generate_delivery(self):
        ...
        name = (self.type == 'sell' and '销售发货单' or '销售退货单')
        return {
            'name': name,
            'view_type': 'form',
            'view_mode': 'form',
...
class SellOrderLine(models.Model):
    ...
    order_id = fields.Many2one('sell.order', '订单编号', index=True,
                               ...
                               help='关联订单的编号')
    currency_amount = fields.Float('外币金额', compute=_compute_all_amount,
                                   store=True,
                                   digits=dp.get_precision('Amount'),
                                   help='外币金额')
...
class ApproveMultiSellOrder(models.TransientModel):
    ...
    def fields_view_get(self, view_id=None, view_type='form', toolbar=False, submenu=False):
        ...
        if done_lists:
            raise UserError('销货订单 ' + done_lists + ' 已确认!')
        return res
