...
class CustomerStatementsReportWithGoods(models.TransientModel):
    ...
    quantity = fields.Float('数量', digits=dp.get_precision('Quantity'))
    price = fields.Float('单价', digits=dp.get_precision('Price'))
    discount_amount = fields.Float('折扣额', digits=dp.get_precision('Amount'))
    without_tax_amount = fields.Float(
        '不含税金额', digits=dp.get_precision('Amount'))
    tax_amount = fields.Float('税额', digits=dp.get_precision('Amount'))
    order_amount = fields.Float(
        string='销售金额', digits=dp.get_precision('Amount'))
    benefit_amount = fields.Float(
        string='优惠金额', digits=dp.get_precision('Amount'))
    fee = fields.Float(string='客户承担费用', digits=dp.get_precision('Amount'))
    amount = fields.Float(string='应收金额', digits=dp.get_precision('Amount'))
    pay_amount = fields.Float(
        string='实际收款金额', digits=dp.get_precision('Amount'))
    discount_money = fields.Float(string='收款折扣', readonly=True,
                                  digits=dp.get_precision('Amount'))
    balance_amount = fields.Float(
        string='应收款余额', digits=dp.get_precision('Amount'))
