...
class test_month_product_cost(TransactionCase):
    ...
    def test_generate_issue_cost(self):
        ...
        wh_in_rows.write({'date':'2016-01-18 18:00:00'})
        wh_out_rows = self.env['wh.out'].search([('name', '=', self.env.ref('warehouse.wh_out_whout1').name)])
        wh_out_rows.write({'date':'2016-01-18 18:00:00'})
        ...
        self.env['month.product.cost'].generate_issue_cost(self.env['finance.period'].get_date_now_period_id())
