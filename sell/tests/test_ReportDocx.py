...
class test_ReportDocx(TransactionCase):
    ...
    def setUp(self):
        ...
        self.ir_actions = self.env.ref('report_docx.report_sell_order_1')
        self.report_docx_sell = self.ir_actions._lookup_report('sell.order')

    def test_lookup_report(self):
        '''测试docx报表模板'''
        self.report_docx_sell.create(self.cr, self.uid, self.ir_actions.id, self.ir_actions, self.env.context)

    def test_lookup_report_pdf(self):
        '''测试docx报表模，输出类型为pdf'''
        self.ir_actions.output_type = 'pdf'
        self.report_docx_sell.create(self.cr, self.uid, self.ir_actions.id, self.ir_actions, self.env.context)
