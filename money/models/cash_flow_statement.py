...
class cash_flow_template(models.Model):
    ...
    # for type sum
    category_ids = fields.Many2many('core.category',string=u'收支类别',domain="[('type','in',['other_get','other_pay'])]")
    # for type begin
    begin_ids = fields.Many2many('finance.account', string=u'会计科目期初')
    # for type end
    end_ids = fields.Many2many('finance.account', string=u'会计科目期末')
    # for type lines
    plus_ids = fields.Many2many('cash.flow.template','c_p','c_id', 'p_id', string=u'+表行')
    nega_ids = fields.Many2many('cash.flow.template','c_n','c_id', 'n_id', string=u'-表行')

class cash_flow_statement(models.Model):
