...
class weixin_contacts(models.Model):
    ...
    department = fields.Char(u'部门', default=lambda self:self.env.ref('gooderp_weiin.weixin_department_user').department_id)
    ...
    active = fields.Boolean(u'有效', deafult=True, help=u'如果微信端被删除，则actived置为False，默认值为True（作用是保留微信端被删除的副本）')
