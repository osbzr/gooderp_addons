...
class AppThemeConfigSettings(models.TransientModel):
    ...
    _description = "App Odoo Customize settings"
    ...
    def set_default_all(self):
        self.ensure_one()
        ir_config = self.env['ir.config_parameter']
        ir_config.sudo().set_param("app_system_name", self.app_system_name or "")
        ir_config.sudo().set_param("app_show_lang", self.app_show_lang or "False")
        ir_config.sudo().set_param("app_show_debug", self.app_show_debug or "False")
        ir_config.sudo().set_param("app_show_documentation", self.app_show_documentation or "False")
        ir_config.sudo().set_param("app_show_documentation_dev", self.app_show_documentation_dev or "False")
        ir_config.sudo().set_param("app_show_support", self.app_show_support or "False")
        ir_config.sudo().set_param("app_show_account", self.app_show_account or "False")
        ir_config.sudo().set_param("app_show_enterprise", self.app_show_enterprise or "False")
        ir_config.sudo().set_param("app_show_share", self.app_show_share or "False")
        ir_config.sudo().set_param("app_show_poweredby", self.app_show_poweredby or "False")

        ir_config.sudo().set_param("app_documentation_url",
                            self.app_documentation_url or "http://www.sunpop.cn/documentation/user/10.0/en/index.html")
        ir_config.sudo().set_param("app_documentation_dev_url",
                            self.app_documentation_dev_url or "http://www.sunpop.cn/documentation/10.0/index.html")
        ir_config.sudo().set_param("app_support_url", self.app_support_url or "http://www.sunpop.cn/trial/")
        ir_config.sudo().set_param("app_account_title", self.app_account_title or "My Online Account")
        ir_config.sudo().set_param("app_account_url", self.app_account_url or "http://www.sunpop.cn/my-account/")
