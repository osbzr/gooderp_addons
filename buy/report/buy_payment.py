...
class BuyPayment(models.TransientModel):
    ...
    purchase_amount = fields.Float('采购金额', digits=dp.get_precision('Amount'))
    discount_amount = fields.Float('优惠金额', digits=dp.get_precision('Amount'))
    amount = fields.Float('成交金额', digits=dp.get_precision('Amount'))
    payment = fields.Float('已付款', digitse=dp.get_precision('Amount'))
    balance = fields.Float('应付款余额', digits=dp.get_precision('Amount'))
