...
测试采购订单
    测试_compute_amount
    测试_get_buy_goods_state
    测试onchange_discount_rate
    测试buy_order_done
    测试buy_order_draft
    测试get_receipt_line
    测试buy_generate_receipt
测试采购订单行
    测试_default_warehouse
    测试_compute_all_amount
    测试onchange_goods_id
    测试onchange_discount_rate
测试采购入库/退货单
    测试_compute_all_amount
    测试_get_buy_money_state
    测试_get_buy_return_state
    测试onchange_discount_rate
    测试get_move_origin
    测试create
    测试buy_receipt_done
...
class test_buy_order(TransactionCase):
    ...
    def test_get_buy_goods_state(self):
        ...
        receipt = self.env['buy.receipt'].search([('order_id', '=', order_copy_1.id)])
...
class test_buy_order_line(TransactionCase):
    ...
    def test_onchange_goods_id(self):
        ...
        for line in self.order.line_ids:
            ...
            self.assertTrue(line.warehouse_dest_id.id == self.env.ref('warehouse.hd_stock').id)
...
class test_buy_receipt(TransactionCase):
    ...
    def setUp(self):
        ...
        self.order = self.env.ref('buy.buy_order_1')
        self.order.buy_order_done()
        self.receipt = self.env['buy.receipt'].search([('order_id', '=', self.order.id)])
