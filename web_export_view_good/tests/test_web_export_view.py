...
class TestWebExportViewTwo(TransactionCase):
    ...
    def test_web_export_view(self):
        ...
        data = {'headers': ['\u5ba2\u6237', ' ', ' ', ' '],
                'model': 'partner',
                'rows': [['\u5ba2\u6237\u7c7b\u522b', '\u7f16\u53f7', '\u540d\u79f0', '\u5e94\u6536\u4f59\u989d'],
                          ['\u4e00\u7ea7\u5ba2\u6237', 'jd', '\u4eac\u4e1c', 0], ['\u5408\u8ba1', ' ', ' ', 0], ['\u64cd\u4f5c\u4eba', 'admin', '\u64cd\u4f5c\u65f6\u95f4', '2016-05-05']],
                'file_address': "%s%s" % (os.path.split(os.path.realpath(__file__))[0], "/hello.xls"),
                'blank_rows':1}
        a = ExcelExportView()
        a.from_data_excel(data.get('headers'), [
                          data.get("rows"), data.get("file_address")])
        data.update({'rows': [['\u5ba2\u6237\u7c7b\u522b', '\u7f16\u53f7', '\u540d\u79f0', '\u5e94\u6536\u4f59\u989d']],
                     'file_address': "%s%s" % (os.path.split(os.path.realpath(__file__))[0], "/None.xls"),
                     'blank_rows':1})
        a.from_data_excel(data.get('headers'), [
                          data.get("rows"), data.get("file_address")])
        data_two = {"model": "res.users", "headers": ["用户", " ", " ", " "],
                    "rows": [["名称", "登录", "语言", "最后一次登录"],
                             ["Administrator", "admin",
                                 "Chinese (CN) / 简体中文", "2016年05月09日"],
                             ["Demo User", "demo", "Chinese (CN) / 简体中文", ""],
                             ["操作人", "admin", "操作时间", "2016-05-09"]],
                    "file_address": '',
                    "blank_rows":1}
...
class TestReportTemplate(TransactionCase):
    ...
    def test_compute_model_name(self):
        template = self.env['report.template'].create(
            {'active': True, 'model_id': self.env.ref('base.model_res_users').id, 'file_address': '/hello.xls', 'blank_rows':1})
