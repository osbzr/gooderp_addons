    ...
    def button_ok(self):
        ...
        for wizard in self:
            ...
            for line in wizard.sell_line_ids:
                if line.order_id not in order_dict:
                    order_dict[line.order_id] = line
                else:
                    order_dict[line.order_id] += line
            if len(list(order_dict.keys())) > 1:
                raise UserError('一次只能勾选同一张销货订单的行')
            for line in wizard.sell_line_ids:
                buy_lines.append(self._get_vals(order, line))
                line.is_bought = True
            # 将销货订单行复制到购货订单
            order.write({
                'sell_id': list(order_dict.keys())[0].id,
