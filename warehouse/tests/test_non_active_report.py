...
class test_non_active_report(TransactionCase):
    def setUp(self):
        ''' 呆滞料报表 测试 '''
        super(test_non_active_report, self).setUp()
    ...
    def test_open_non_active_report_warehouse(self):
        ...
        self.keyboard_mouse_in_line.date = datetime.datetime.now() - datetime.timedelta(days=1)
        ...
        fir_non_active_report = self.env['non.active.report'].search([('goods_id', '=', self.goods_keyboard_mouse)])
        self.assertEqual(len(fir_non_active_report), 1)
        sec_non_active_report = self.env['non.active.report'].search([('goods_id', '=', self.goods_keyboard)])
