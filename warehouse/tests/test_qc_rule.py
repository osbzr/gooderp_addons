...
class test_qc_rule(TransactionCase):
    ...
    def test_compute_warehouse_impl(self):
        ...
        self.assertTrue(self.rule_wh_in_inv.warehouse_id == self.warehouse_inventory)
        ...
        self.assertTrue(self.rule_sell_return.warehouse_id == self.warehouse_customer)

    def test_compute_warehouse_dest_impl(self):
        ''' 根据单据类型自动填充上调入仓库 '''
        self.rule_sell.onchange_move_type()
        self.assertTrue(self.rule_sell.warehouse_dest_id == self.warehouse_customer)
        self.rule_buy_return.onchange_move_type()
        self.assertTrue(self.rule_buy_return.warehouse_dest_id == self.warehouse_supplier)
        self.rule_wh_out.onchange_move_type()
        self.assertTrue(self.rule_wh_out.warehouse_dest_id == self.warehouse_others)
        self.rule_wh_out_inv.onchange_move_type()
        self.assertTrue(self.rule_wh_out_inv.warehouse_dest_id == self.warehouse_inventory)
