...
class report_stock_transceive(models.Model):
    ...
    goods_qty_begain = fields.Float(
        '期初数量', digits_compute=dp.get_precision('Goods Quantity'))
    cost_begain = fields.Float(
        u'期初成本', digits_compute=dp.get_precision('Accounting'))
    goods_qty_end = fields.Float(
        u'期末数量', digits_compute=dp.get_precision('Goods Quantity'))
    cost_end = fields.Float(
        u'期末成本', digits_compute=dp.get_precision('Accounting'))
    goods_qty_out = fields.Float(
        u'出库数量', digits_compute=dp.get_precision('Goods Quantity'))
    cost_out = fields.Float(
        u'出库成本', digits_compute=dp.get_precision('Accounting'))
    goods_qty_in = fields.Float(
        '入库数量', digits_compute=dp.get_precision('Goods Quantity'))
    cost_in = fields.Float(
        u'入库成本', digits_compute=dp.get_precision('Accounting'))
