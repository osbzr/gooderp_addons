...
class checkout_wizard(models.TransientModel):
    ...
    period_id = fields.Many2one('finance.period',u'结账会计期间',required=True)
    date = fields.Date(u'生成凭证日期')
    ...
    def button_checkout(self):
        if self.period_id:
            ...
            if self.period_id.month == '12':
                ...
                year_line_ids=[{
                     'name':u'年度结余',
                     'account_id':company_obj.search([])[0].no_profit_account.id,
                     'debit':0,
                     'credit':year_total,
                     },{
                     'name':u'年度结余',
            ...
            if not next_period:
                if self.period_id.month == '12':
                    self.env['finance.period'].create({'year':self.period_id.year +1,
                                                       'month':1,})
                else:
                    self.env['finance.period'].create({'year':self.period_id.year,
                                                       'month':self.period_id.month + 1 ,})
            #显示凭证
