        ...
        if self.period_id:
            ...
            if last_period:
                if not last_period.is_closed:
                    raise UserError(u'错误', u'上一个期间%s未结账' % last_period.name)
            if self.period_id.is_closed:
                raise UserError(u'错误', u'本期间已结账')
            else:
                ...
                for voucher_id in voucher_ids:
                    if voucher_id.state != 'done':
                        i += 1
                if i != 0:
                    raise UserError(u'错误', u'该期间有%s张凭证未审核' % i)
                else:
                    ...
                    if not year_profit_account:
                        raise UserError(u'错误', u'公司本年利润科目未配置')
                    if not remain_account:
                        raise UserError(u'错误', u'公司未分配利润科目未配置')
    ...
    def button_counter_checkout(self):
        if self.period_id:
            if not self.period_id.is_closed:
                raise UserError(u'错误', u'本期间未结账')
            else:
                next_period = self.env['create.trial.balance.wizard'].compute_next_period_id(self.period_id)
                if next_period:
                    if next_period.is_closed:
                        raise UserError(u'错误', u'下一个期间%s已结账！' % next_period.name)
                ...
                if last_period:
                    while last_period and last_voucher_number < 1:
                        ...
                        if period_id.year != last_period.year:
                            ...
                            #else:
                            #    raise UserError(u'错误', u'请核实上一个期间：%s是否有凭证！' % last_period.name)
