...
class staff_hire_stage(models.Model):
    ...
    name = fields.Char("阶段名", required=True)
    sequence = fields.Integer(
        "序号", default=10,
        help="按顺序显示列表中的各阶段。")
    job_id = fields.Many2one('staff.job', string='具体职位',
                             ondelete='cascade',
                             help='使用此阶段的具体职位。其他职位将不使用此阶段。')
    requirements = fields.Text("要求")
    template_id = fields.Many2one(
        'mail.template', "使用模板",
        help="如果设置,当应聘者设置此场景时,将通过这个模板推送消息给相关应聘者")
    fold = fields.Boolean(
        "在招聘管道收起",
        help="当这个阶段中没有任何记录要呈现的时候，这个阶段在看板视图中被折叠起来")
...
class hire_applicant(models.Model):
    ...
    active = fields.Boolean("有效", default=True, help="如果“有效”字段设为false，它对信息进行隐藏但不删除它。")
    note = fields.Text("备注")
    email_from = fields.Char("Email", size=128, help="这些人将收到电子邮件")
    stage_id = fields.Many2one('staff.hire.stage', '阶段', track_visibility='onchange',
                               domain="['|', ('job_id', '=', False), ('job_id', '=', job_id)]",
                               copy=False, index=True,
                               group_expand='_read_group_stage_ids',
                               default=_default_stage_id)
    last_stage_id = fields.Many2one('staff.hire.stage', "最终阶段",
                                    help="当前阶段前的申请人阶段，用于招聘失败分析。")
    categ_ids = fields.Many2many('core.value', string="标签",
                                 domain=[('type', '=', 'hire_categ')],
                                 context={'type': 'hire_categ'}
                                 )
    company_id = fields.Many2one('res.company', "公司", default=lambda self: self.env['res.company']._company_default_get())
    user_id = fields.Many2one('res.users', "负责人", track_visibility="onchange", default=lambda self: self.env.uid)
    date_last_stage_update = fields.Datetime("最终阶段更新时间", index=True, default=fields.Datetime.now)
    date_action = fields.Date("下步计划日期")
    title_action = fields.Char("下步计划", size=64)
    priority = fields.Selection(AVAILABLE_PRIORITIES, "欢迎度", default='0')
    job_id = fields.Many2one('staff.job', "应聘职位", required=True)
    salary_proposed_extra = fields.Char("建议额外福利", help="除了建议薪资以外的福利待遇")
    salary_expected_extra = fields.Char("期望额外福利", help="除了期望薪资以外的福利待遇")
    salary_proposed = fields.Float("建议薪资", help="该公司的提议薪酬")
    salary_expected = fields.Float("期望薪资", help="申请人要求的薪酬")
    date_available = fields.Date("开始工作日期", help="申请人能够开始工作的日期")
    partner_name = fields.Char("应聘者姓名", required=True)
    partner_mobile = fields.Char("手机", size=32, required=True)
    degree_id = fields.Many2one('core.value', "学历",
                              domain=[('type', '=', 'hire_degree')],
                              context={'type': 'hire_degree'}
                              )
    department_id = fields.Many2one('staff.department', "部门")
    reference = fields.Char("推荐人")
    color = fields.Integer("Color Index", default=0)
    staff_id = fields.Many2one('staff', string="员工", track_visibility="onchange", help="关联员工")
    staff_name = fields.Char(related='staff_id.name', string="员工姓名")
    user_email = fields.Char(related='user_id.email', type="char", string="用户Email", readonly=True)
    attachment_number = fields.Integer(compute='_get_attachment_number', string="附件数量")
    attachment_ids = fields.One2many('ir.attachment', 'res_id', domain=[('res_model', '=', 'hire.applicant')], string='附件')
    source_id = fields.Many2one('core.value', "来源", ondelete='restrict',
