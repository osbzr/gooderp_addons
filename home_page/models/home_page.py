...
class HomeReportType(models.Model):
    ...
    name = fields.Char('报表类别', help='在home.page 中报表类别不同类别的可以分组,组别的名称!')
    sequence = fields.Integer('序列', help='在home.page 中报表类别分组 时的顺序进行调整!')
...
class HomePage(models.Model):
    _name = "home.page"
    _rec_name = "note_one"
    _description = "本模块就是一些常用操作的集合,也是小量级各方数据显示的平台！"
    _order = "sequence"

    sequence = fields.Integer('序号', help='用来确定 每条记录在首页中的显示的顺序(而不仅仅是记录的顺序)')
    action = fields.Many2one('ir.actions.act_window', string='动作', required='1', help='设置首页点击事件,\
                                                                                         跳转对应的模型action')
    menu_type = fields.Selection([('all_business', '主要功能'), ('amount_summary', '关键指标'), ('report', '常用报表')],
                                 string='类型', required="1", help='选定本条记录的类型,本字段会决定本条记录属于那一块')
    domain = fields.Char('过滤', default='[]',
                         help='字符串条件,用来过滤出您所选的视图中的数据!')
    note_one = fields.Char('名称', help='在首页中的相应的文字显示内容')
    compute_field_one = fields.Many2one('ir.model.fields', string='计算字段', help='在首页中有用于数字显示的元素的,\
                                                            仅适合小量数据的计算数字对应的视图或模型中的字段!')
    compute_type = fields.Selection([('sum', 'sum'), ('average', 'average')], default="sum", string="计算方式",
                                    help='对于所选的计算字段的计算方式!注:目前只支持sum')
    context = fields.Char('上下文', help='对应跳转视图传进去的参数!')
    is_active = fields.Boolean(
        '可用', default=True, help='为了方便调试,首页美观性,或临时性替换首页元素!')
    report_type_id = fields.Many2one('home.report.type', string='报表类别', help='类型为 常用报表时 要选择报表类别,\
                                    可以对不同类型的报表进行分组!')
    ...
    def get_action_url(self):
        ...
        action_url_list['right'] = sorted(
            list(action_url_list['right'].items()), key=lambda d: d[0])
